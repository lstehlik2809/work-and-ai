...
Source URLs, actual retrieval timestamps, release identifiers, licenses, raw SHA-256 digests, stored digests and sizes are in [data/source-manifest.json](data/source-manifest.json). That original manifest covers three XLSX files and four losslessly gzipped CSV files under `data/raw`. The separate [data/skills-source-manifest.json](data/skills-source-manifest.json) covers three additional gzipped O*NET CSV sources, with their own source URLs, retrieval timestamps, hashes, sizes, release and license. An additional original BLS `occupation.xlsx` workbook and full Table 1.12 JSON extraction are tracked separately in [data/ai-context-source-manifest.json](data/ai-context-source-manifest.json) for optional AI outlook context. Only successfully parsed downloads are included. Normal site builds use committed outputs and do not fetch sources.
...
## Optional AI outlook context

[The BLS occupational workbook](https://www.bls.gov/emp/ind-occ-matrix/occupation.xlsx), Table 1.12, contains 935 valid occupation–industry staffing-factor rows for 2025–2035. The separate source manifest pins workbook SHA-256 `a7d060f0576f7d4f5b829b9a0bb4a6c11a12766d159ebb0e07aab5bd2e7583c5`, the completed retrieval timestamp, size, URL and public-government-data license. `scripts/data/extract-ai-context.py` validates the sheet title, headers and rows, and preserves all 935 rows in `data/ai-context-table.json`; its exact UTF-8 bytes have a second hash pin. `scripts/data/ai-context.mjs` selects explicit AI terms, verifies every selected occupation code/title against the existing BLS snapshot, and writes the 56-note, 50-occupation public artifact plus its browser integrity pin. No original exposure, skills, occupational or semantic asset is regenerated by this pipeline.

Reproduce offline with `npm run ai-context:extract` (Python/openpyxl), then `npm run ai-context:prepare` (Node). Repeat extraction must produce the same pinned intermediate bytes. `npm run validate:ai-context` and `npm test` validate hashes, exact derivation and runtime pin without Python or network. `npm run ai-context:fetch` deliberately reacquires only the existing pinned workbook; changed upstream bytes fail rather than silently updating the release. The [result-context methodology](METHODOLOGY.md#result-context-category-evidence-dates-and-ai-outlook-notes) explains the explicit-term subset, multiple notes, industry staffing-share scope, absence, and interpretation limits.
