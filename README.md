...
Normal builds use committed public data and vectors plus prepared, pinned model/runtime files. Run `npm run semantic:fetch` after checkout. CI performs the same verified acquisition before building. They neither contact the upstream data services nor re-embed the corpus. React, TypeScript and Vite render a static site; Fuse.js and BM25 provide ordinary retrieval. No server, accounts, analytics, fonts, inference API, or generative interpretation service is used.

## Release scope

The current release adds a continuous-skill occupation map with descriptive connections, descriptive skill-pattern comparisons in a sortable five-column table, and direct PDF downloads for occupations and comparisons. It preserves the live MiniLM model, index and search ranking. See the [current methodology](METHODOLOGY.md) for map similarity, descriptive calculations and coverage limits. The stronger-model experiment and editorial title additions are deferred. Their original failures remain recorded in [MPNet implementation verification](verification/mpnet-implementation.md) and the [model comparison](verification/embedding-model-comparison.md); neither is evidence of improved production matching. The [13 September release verification](verification/live-improvements.md) records the earlier release. Only `main` deploys Pages.
...
Build-time CPU inference uses the same quantized MiniLM ONNX weights, tokenizer, 256-token boundary, mean pooling, normalization and 384 dimensions as the browser. The original builder selects whole source fragments and at most three evenly spaced role passages per canonical occupation, recording omissions. The production model, 952 passages and index are preserved from the prior live release. It writes one normalized centroid per canonical occupation and a first-passage ablation index. `data/semantic-passages.json` and `data/evaluation/mpnet/` retain the separate frozen model-comparison evidence. Review changed source coverage and run all checks before any future index update.
...
The **Occupation map** view gives an overview of relative AI exposure across occupations with O*NET skill evidence. It plots the 771 BLS occupations with at least 20 rated skills from the 831-occupation snapshot, with ordered circle sizes and colors for the four exposure categories and a distinct hollow mark for unavailable exposure. **Highlight on map** highlights title/code matches or occupations where a selected skill is important, with dark rings while retaining other occupations as context; only the list narrows to those matches. Exposure toggles show any combination of categories. **All** restores every category; turning off the last selected category or selecting all five also returns to All. Neither interaction changes positions. The chart has no guide axes; its lines connect selected skill neighbors. The list shows higher exposure categories first and sorts alphabetically within a category.
...
Select an occupation to see its important skills and up to five closest matches based on continuous importance ratings. Pairs need at least 20 jointly rated skills; similarity is 100% minus the average absolute rating difference as a percentage of the four-point scale range. Details show the number of jointly rated skills and their paired ratings, displayed to two decimal places. **View occupation details** opens the existing occupation view for comparison and source context. The map lazy-loads the existing skills artifact and its UI, requires no model or new dependency, and makes no external-service requests. Failed loads offer a retry.

The map uses **UMAP** fitted from continuous importance-rating differences, with a small deterministic spacing adjustment to make crowded circles easier to distinguish. The adjustment is independent of exposure, runs once for the full population, and keeps each circle near its original position. Search, filters and zoom leave positions fixed; connections use the same continuous rating similarity as the layout, calculated from the original ratings rather than distances on the screen. Missing ratings are omitted from each pairwise comparison. The map offers a retry if its saved layout cannot load or fails validation.

The original [UMAP/PCA comparison report](data/projection-evaluation/README.md) records all 27 runs, seed stability and reproduction commands. Its 52.7% UMAP versus 30.3% PCA neighborhood scores describe the earlier binary-skill layouts before the added spacing adjustment, not the current continuous-rating layout; they are neither predictive accuracy nor AI exposure. PCA is retained only in the offline comparison code. Both projection and visual spacing are approximate, so cluster gaps, density and global distances should not be read quantitatively. See [METHODOLOGY.md](METHODOLOGY.md#occupation-map) for the full rules. Run `npx tsx --test tests/occupation-map*.test.ts` for focused checks and `npm run browser:map` against the production preview for interactive checks. UMAP fitting is offline and adds no browser dependency or model download.


## Analysis controls and reproducibility

Skill patterns show descriptive counts, prevalence ratios and percentage-point differences in a sortable five-column table, initially ordered by descending prevalence ratio. Rated/eligible coverage appears alongside important/rated counts. The table uses a fixed importance threshold of 3 / 5. Sorting the observed-difference column uses absolute magnitude while retaining signed values. Downloads keep absolute-difference ordering regardless of the table sort. Open the methodology disclosure for coverage and limitations, and download the sensitivity comparisons for alternative thresholds, specialty aggregation, coverage restrictions and occupational-family omissions. Downloads contain all 35 skills regardless of private text filters. Confirmed occupations and comparisons offer a public reading brief and direct PDF downloads.

The map uses continuous importance comparisons for both geometry and connections, with at least 20 jointly rated skills per pair. There is no comparison-method selector; search, exposure filters and camera controls leave the layout fixed.
