...
Select an occupation to see its important skills and up to five closest matches based on continuous importance ratings. Pairs need at least 20 jointly rated skills; similarity is 100% minus the average absolute rating difference as a percentage of the four-point scale range. Details show the number of jointly rated skills and their exact paired ratings. **View occupation details** opens the existing occupation view for comparison and source context. The map lazy-loads the existing skills artifact and its UI, requires no model or new dependency, and makes no external-service requests. Failed loads offer a retry.
...
Skill patterns show descriptive counts, prevalence ratios and percentage-point differences in a sortable five-column table, initially ordered by absolute difference. Rated/eligible coverage appears alongside important/rated counts. The table uses a fixed importance threshold of 3 / 5. Open the methodology disclosure for coverage and limitations, and download the sensitivity comparisons for alternative thresholds, specialty aggregation, coverage restrictions and occupational-family omissions. Downloads contain all 35 skills regardless of private text filters. Confirmed occupations and comparisons offer a public reading brief and direct PDF downloads.
