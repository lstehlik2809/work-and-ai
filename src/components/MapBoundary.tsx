...
export default class MapBoundary extends Component<{children: ReactNode}, {failed: boolean}> {
  ...
  render() {
    if (this.state.failed) return <div role="alert"><p>The occupation map could not load. Reload the page to retry. Occupation search remains available.</p><button className="secondary" onClick={() => location.reload()}>Reload map</button></div>;
