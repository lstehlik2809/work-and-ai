...
function PatternTable({snapshot,skills,artifact}:{snapshot:Snapshot;skills:SkillsData;artifact:PatternArtifact}){
 ...
 const analysis={totalOccupations:artifact.total,eligibleOccupations:variant.eligible,unknownExposure:artifact.unknownExposure,noRatings:artifact.noRatings,categoryCounts:variant.categoryCounts};
  ...
  return <section className="skill-patterns" aria-labelledby="patterns-heading">
    <TabHeader id="patterns-heading" eyebrow="Skills across occupations" title="Occupational skill profiles.">Choose an AI exposure category to explore its skill profile. Compare how often each skill is important there versus all occupations with known exposure or the other exposure categories.</TabHeader>
    ...
    <div className="patterns-toolbar"><div><h2>Important skills, in context</h2><p id="patterns-order">The default shows the highest prevalence ratios first. Select a column header to sort; select it again to reverse the order. Observed difference sorts by absolute size, while the values retain their positive or negative signs. Unavailable or undefined values appear last. Prevalence ratio = selected ÷ {baselineLabel.toLowerCase()} prevalence. Above 1× = more common; below 1× = less common.</p></div><div><label htmlFor="patterns-search">Filter skills</label><input id="patterns-search" type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Name, group or definition"/></div></div>
    ...
    <p className="hint">The main download contains all 35 skills for the selected category and baseline at importance ≥ 3 / 5, ordered by absolute percentage-point difference. The sensitivity download adds alternative thresholds, specialty aggregation and coverage rules. Both ignore your text filter and table sort order.</p>
