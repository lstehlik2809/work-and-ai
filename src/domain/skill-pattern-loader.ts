...
import{PATTERN_METHOD,PATTERN_MODEL,PATTERN_PARAMETERS}from'./skill-pattern-data';
...
export function validatePatternArtifact(value:unknown,snapshot:Snapshot,skills:SkillsData):PatternArtifact{
 ...
 if(!a||a.schemaVersion!==1||a.release!==snapshot.release.id||a.release!==pin.release||a.method!==PATTERN_METHOD||a.model!==PATTERN_MODEL||!same(a.parameters,PATTERN_PARAMETERS)||!same(a.sourceHashes,pin.sourceHashes)||a.generationCodeHash!==pin.generationCodeHash||!same(a.skillIds,skills.skills.map(s=>s.id))||a.total!==snapshot.occupations.length||a.unknownExposure!==snapshot.occupations.filter(o=>o.exposure===null).length||!integer(a.noRatings)||a.noRatings>a.total||!Array.isArray(a.variants)||!same(a.variants.map(v=>v.id),pin.variantIds))bad();
 for(const v of a.variants){
  ...
  for(const r of v.rows){
   ...
   const [yes,no,otherYes,otherNo]=r.counts,p=r.posterior;
   if(v.id.startsWith('mean-')){
    if(!yes&&!no||!otherYes&&!otherNo){if(p!==null)bad();}
    else if(!p||!Number.isFinite(p.probabilityMore)||p.probabilityMore<0||p.probabilityMore>1||!same(p.selectedPosterior,[yes+1,no+1])||!same(p.otherPosterior,[otherYes+1,otherNo+1])||!Number.isFinite(p.meanDifference)||Math.abs(p.meanDifference-100*((yes+1)/(yes+no+2)-(otherYes+1)/(otherYes+otherNo+2)))>1e-10||!Array.isArray(p.interval)||p.interval.length!==2||p.interval.some(x=>!Number.isFinite(x)||x< -100||x>100)||p.interval[0]>p.interval[1]||p.constantOutcome!==(yes+otherYes===0||no+otherNo===0))bad();
   }else if(p!==undefined)bad();
  }
