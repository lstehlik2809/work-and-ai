...
export const PATTERN_METHOD='occupational-skill-patterns-v1';
export const PATTERN_MODEL='independent-beta-binomial-v1';
export const PATTERN_PARAMETERS={thresholds:[2.5,3,3.5],primaryAggregation:'mean',alternativeAggregation:'max',minimumCoverage:28,skillCount:35,sparseRatedBelow:30,sparseImportantBelow:10,directionBoundaryPp:1,magnitudeRangePp:5,topCount:5,draws:20000,prior:[1,1]};
export interface PatternRow{category:Exposure;skillId:string;counts:Contingency;selectedEligible:number;otherEligible:number;posterior?:BayesianEvidence|null}
...
export interface PatternArtifact{schemaVersion:1;release:string;method:string;model:string;sourceHashes:{occupations:string;skills:string};generationCodeHash:string;parameters:typeof PATTERN_PARAMETERS;skillIds:string[];total:number;unknownExposure:number;noRatings:number;variants:PatternVariant[]}
...
export function projectPatterns(artifact:PatternArtifact,skills:SkillsData,category:Exposure,baseline:SkillBaseline,threshold:number){
 ...
 return variant.rows.filter(r=>r.category===category).map(r=>({...selectSkillBaseline(buildSkillExposureRow(skills.skills.find(s=>s.id===r.skillId)!,category,r.counts,r.posterior??null),baseline),selectedEligible:r.selectedEligible,baselineEligible:baseline==='overall'?r.selectedEligible+r.otherEligible:r.otherEligible}));
}
export function patternSensitivity(artifact:PatternArtifact,skills:SkillsData,category:Exposure,baseline:SkillBaseline){
 const variants=artifact.variants.map(v=>{const rows=v.rows.filter(r=>r.category===category).map(r=>selectSkillBaseline(buildSkillExposureRow(skills.skills.find(s=>s.id===r.skillId)!,category,r.counts,null),baseline));return{variant:v,rows,top:new Set(sortPatterns(rows).filter(r=>r.difference!==null).slice(0,5).map(r=>r.skill.id))};});
