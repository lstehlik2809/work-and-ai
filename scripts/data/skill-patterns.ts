...
import{buildPatternCounts,PATTERN_METHOD,PATTERN_MODEL,PATTERN_PARAMETERS}from'../../src/domain/skill-pattern-data';
...
for(const v of variants)if(v.id.startsWith('mean-'))for(const row of v.rows)row.posterior=bayesianPrevalence(row.counts);
const sourceHashes={occupations:hash('public/data/occupations.json'),skills:hash('public/data/skills.json')};
const generationCodeHash=createHash('sha256').update(['scripts/data/skill-patterns.ts','src/domain/skill-pattern-data.ts','src/domain/skill-exposure.ts','src/domain/bayesian-prevalence.ts','src/domain/occupation-map.ts'].map(p=>p+':'+hash(p)).join('\n')).digest('hex');
const artifact={schemaVersion:1,release:snapshot.release.id,method:PATTERN_METHOD,model:PATTERN_MODEL,sourceHashes,generationCodeHash,parameters:PATTERN_PARAMETERS,skillIds:skills.skills.map((s:{id:string})=>s.id),total:snapshot.occupations.length,unknownExposure:snapshot.occupations.filter((o:{exposure:unknown})=>o.exposure===null).length,noRatings:buildOccupationProfiles(snapshot,skills).excludedCodes.filter(c=>snapshot.occupations.find((o:{code:string})=>o.code===c).exposure!==null).length,variants};
