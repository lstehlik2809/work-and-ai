...
let releaseImport;
try{
 await returningVisitor(page);await page.goto(base);await page.getByRole('searchbox',{name:'Job title',exact:true}).waitFor();assert(!requests.some(url=>/data\/skills\.json|\/models\/|\/semantic\/|\.wasm/.test(url)));await page.getByRole('searchbox',{name:'Job title',exact:true}).fill('electrical');await tick();
 ...
 let attempts=0;await page.route('**/data/skills.json',route=>++attempts===1?route.fulfill({status:503,body:'Unavailable'}):route.continue());await returningVisitor(page);await page.goto(base);await trigger().click();await page.getByRole('button',{name:'Retry skills reference',exact:true}).waitFor();assert.equal(await cards().count(),0);
 ...
 await page.unroute('**/data/skills.json');attempts=0;await page.route('**/data/skills.json',route=>++attempts===1?route.fulfill({status:503,body:'Unavailable'}):route.continue());await returningVisitor(page);await page.goto(base);await trigger().click();await page.getByRole('button',{name:'Retry skills reference',exact:true}).click();await select('Programming');await tick();assert.equal(attempts,2);
 report.checks.push('503 reference failure leaves title usable; reopening and explicit Retry both recover');
 await page.unroute('**/data/skills.json');let sourceSeen,releaseSource;const sourceRequested=new Promise(resolve=>sourceSeen=resolve),sourceReleased=new Promise(resolve=>releaseSource=resolve);await page.route('**/data/skills.json',async route=>{sourceSeen();await sourceReleased;await route.continue();});await returningVisitor(page);await page.goto(base);await trigger().click();await sourceRequested;await page.getByRole('button',{name:'Close skills panel'}).click();releaseSource();await page.waitForLoadState('networkidle');assert.equal(await page.locator('#skills-panel').count(),0);assert.equal(await cards().count(),0);await trigger().click();await select('Programming');await tick();assert.match(await cards().first().innerText(),/Computer Programmers/);
